...
def find_encoding(file):
    """
    Функция поиска кодировки файла с помощью библиотеки chardet, для
    последующей передачи в параметр 'encoding='
    :param file:
...
def get_data(lst):
    """
    Функция для считывания данных из принятого списка файлов, и последующего
    формирования выходных данных в соответствии с задаными параметрами
    :param lst:
    :return:
    """
    os_prod_list = []
        ...
        with open(file, 'rb') as r_f:
            for line in r_f:
                ...
                if re.match(main_data[0][0], row):
                    os_prod_list.append(re.search(
                        r'(Изготовитель системы).\s*(.*)', row).group(2))
                elif re.match(main_data[0][1], row):
                    os_name_list.append(re.search(
                        r'(Название ОС).\s*(.*)', row).group(2))
                elif re.match(main_data[0][2], row):
                    os_code_list.append(re.search(
                        r'(Код продукта).\s*(.*)', row).group(2))
                elif re.match(main_data[0][3], row):
                    os_type_list.append(re.search(
                        r'(Тип системы).\s*(.*)', row).group(2))
...
if __name__ == '__main__':
    ...
    with open('data_file.csv', encoding='utf-8') as fl:  # чтение файла
        print(fl.read())
