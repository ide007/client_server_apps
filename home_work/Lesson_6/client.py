...
def main():
    ...
    client_logger.debug(f'Создан клиентский сокет.')
    ...
    try:
        ...
        print(answer)
        client_logger.info(f'Получено сообщение "{answer}" от сервера.')
        client_socket.close()
        client_logger.debug(f'Клиентский сокет закрыт.')
